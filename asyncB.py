...
class AsyncFS:
    ...
    def __init__(self):
        ...
        self.port = self.cfg['port']['b']
        self.dir = self.cfg['dir']['b']
        self.nodes = self.cfg['nodes_for_B']

    def run(self):
        run = web.Application()
        run.add_routes([web.get('/{file}', self.find),
    ...
    async def connecto(self, session, url):
